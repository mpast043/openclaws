...
def run_gap_tail(
    ...
    for eps in epsilons:
        ...
        if np.isnan(r2_exp) or lambda1 < 1e-8:
            fit_winner = "none"
...
def write_summary(rows: List[GapResult]) -> Path:
    ...
    with open(summary_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["epsilon", "lambda_1", "lambda_int", "delta_lambda", "delta_ds_max", "R2_exp", "R2_power", "FitWinner", "Regime"])
        for r in rows:
            writer.writerow([
                f"{r.epsilon:.6f}",
...
def print_table(rows: List[GapResult]):
    header = "| ε | λ₁ | λ_int | Δλ | Δd_s(max) | R²_exp | R²_power | FitWinner | Regime |"
    sep = "|---:|----:|------:|----:|---------:|-------:|---------:|:----------|:-------|"
    print(header)
    print(sep)
    for r in rows:
        print(
            ...
            f"{r.delta_ds_max:.5f} | {r.r2_exp:.3f} | {r.r2_power:.3f} | {r.fit_winner:9} | {r.regime} |")
...
def main():
    ...
    if args.emit:
        run_id = f"gap_tail_n{args.n}_r{args.radius}_s{args.seed}"
        metrics_path, selection_path = emit_gap_tail_artifacts(OUTPUT_DIR, rows, run_id)
        print(f"Validation artifacts written:")
