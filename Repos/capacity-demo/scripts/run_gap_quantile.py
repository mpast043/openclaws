...
from scripts.emitters import ValidationEmitter, emit_gap_quantile_artifacts
...
def run_quantile_sweep(
    ...
    for q in q_list:
        ...
        # Determine fit winner (which model explains the data better)
        if np.isnan(r2_exp):
            fit_winner = "none"
        elif np.isnan(r2_power):
            fit_winner = "exp"
        elif r2_power > r2_exp + 0.001:  # Margin to avoid tie-noise
            fit_winner = "power"
        else:
            fit_winner = "exp"

        # Determine regime (broader classification with stop conditions)
        if r2_exp < 0.9 or np.isnan(r2_exp):
            regime = "Critical"
            stop_reason = "numerical"
        elif delta_lambda < 1.0:
            regime = "Degenerate"
            stop_reason = "gap"
        elif fit_winner == "power":
            regime = "Critical"
            stop_reason = "power"
        else:
            regime = "Rigid"
        ...
        rows.append({
            ...
            "r2_exp": r2_exp,
            "r2_power": r2_power,
            "fit_winner": fit_winner,  # New: explicit fit winner
            "regime": regime,
            "sigma": sigma_window,
...
def write_table(rows: list[dict]) -> Path:
    ...
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["q_int", "lambda1", "lambda_int", "delta_lambda", "delta_ds_max", "R2_exp", "R2_power", "FitWinner", "Regime"])
        for r in rows:
            writer.writerow([
                f"{r['q_int']:.2f}",
                f"{r['lambda1']:.6f}",
...
def main():
    ...
    parser.add_argument("--emit", action="store_true", help="Emit validation artifacts (metrics.json, selection.jsonl)")
    ...
    # Report which q_int values triggered power-law fit wins
    power_wins_at = [r['q_int'] for r in rows if r.get('fit_winner') == 'power']
    if power_wins_at:
        q_list_str = ", ".join(f"{q:.2f}" for q in power_wins_at)
        print(f"Power-law fit wins at q_int in {{{q_list_str}}}")
    else:
        print("Exponential fit wins at all evaluated q_int")

    # Show stop reason separately from fit winner
    if stop_reason == "gap":
        last_q = rows[-1]['q_int'] if rows else 'N/A'
        print(f"Stop triggered at q_int={last_q}: spectral degeneracy (Δλ < 1.0)")
    elif stop_reason == "power":
        last_q = rows[-1]['q_int'] if rows else 'N/A'
        print(f"Stop triggered at q_int={last_q}: R²_power > R²_exp + 0.001")
    elif stop_reason == "numerical":
        last_q = rows[-1]['q_int'] if rows else 'N/A'
        print(f"Stop triggered at q_int={last_q}: numerical (R²_exp < 0.9)")
    else:
        print("No stop condition triggered (completed full q_int list)")

    # Pretty table for console - now shows both FitWinner and Regime
    print("\n| q_int | λ₁ | λ_int | Δλ | Δd_s(max) | R²_exp | R²_power | FitWinner | Regime |")
    print("|-----:|----:|------:|----:|---------:|-------:|---------:|:----------|:-------|")
    for r in rows:
        fit_winner = r.get('fit_winner', 'exp')
        print(
            f"| {r['q_int']:.2f} | {r['lambda1']:.4f} | {r['lambda_int']:.2f} | {r['delta_lambda']:.2f} | "
            f"{r['delta_ds_max']:.5f} | {r['r2_exp']:.3f} | {r['r2_power']:.3f} | {fit_winner:9} | {r['regime']} |")

    # Emit validation artifacts if requested
    if args.emit:
        run_id = f"gap_quantile_r{args.radius}_s{args.seed}"
        metrics_path, selection_path = emit_gap_quantile_artifacts(OUTPUT_DIR, rows, run_id)
        print(f"\nValidation artifacts written:")
        print(f"  metrics.json: {metrics_path}")
        print(f"  selection.jsonl: {selection_path}")
