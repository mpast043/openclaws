...
### Probe budgets (N- and r-dependent)

SLQ variance is highest on small lattices and for aggressive rewiring. The
CLI now assigns probe/Lanczos budgets based on lattice size and rewire rate:

- N ≤ 16: 80 probes / 120 Lanczos iterations
- N ≤ 32: 55 probes / 95 Lanczos iterations
- N ≥ 64: 35 probes / 80 Lanczos iterations

Rewire rates ≥ 0.05 add a 20% probe boost (and ~10% Lanczos boost); rates ≥ 0.08 add a 35% probe boost (and ~21% Lanczos boost). These auto budgets are the default for both presets and custom runs; `--n-probes` / `--m-lanczos` may still override them when needed.

### Boundary diagnostics

Every sweep records each N_equiv jump interval alongside the strongest local R_gamma.
Per-run data live in `split_boundaries.json`; repository-level tables
(`boundary_table.csv`, `boundary_stats_by_N_r.csv`, `boundary_presence_by_rate.csv`)
provide means/uncertainty across seeds and rewire rates.

### Refined C_geo grid

To resolve the recurring brackets near ~0.33 and ~0.70, the CLI injects dense
sub-grids in [0.27, 0.45] and [0.60, 0.78] (24 points per band by default) in
addition to the global linear grid. Use `--no-refine` to disable or
`--refine-band lo hi steps` to customize bands.

### RGG family option

`run_nonseparable_rewire_test.py` now accepts `--family rgg` to swap the base
graph for a random geometric graph. The existing `rates` array controls the
connection radius (scaled by `--rgg-radius-scale`, default 4×). Metadata inside
each run records the actual radius and edge count so you can compare lattice and
RGG staircases side-by-side.
