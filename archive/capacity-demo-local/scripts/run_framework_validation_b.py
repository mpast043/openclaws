...
def _powerlaw_filter_for_substrate(sub: SubstrateResult, d0: float) -> PowerLawFilter:
    """Choose power-law filter parameters tuned for the substrate type."""
    sub_type = sub.metadata.get("type", "")
    if sub_type == "periodic_lattice":
        return PowerLawFilter(d0=d0)
    if sub_type == "random_geometric_graph":
...
def _build_quick_matrix() -> list[RunConfig]:
    ...
    for filt_name, filt_obj in [
        ...
        ("powerlaw", _powerlaw_filter_for_substrate(sub, d0=2.0)),
    ]:
    ...
    configs.append(RunConfig(
        substrate=sub, filter_obj=_powerlaw_filter_for_substrate(sub, d0=2.0),
...
def _build_default_matrix() -> list[RunConfig]:
    ...
    for D, N in [(2, 32), (3, 16)]:
        ...
        for filt_name, filt_obj in [
            ("hard_cutoff", HardCutoffFilter()),
            ("soft_cutoff", SoftCutoffFilter()),
            ("powerlaw", _powerlaw_filter_for_substrate(sub, d0=float(D))),
        ]:
            configs.append(RunConfig(
                substrate=sub, filter_obj=filt_obj,
    ...
    for n_v, D_rgg, r in [(200, 2, 0.35), (150, 3, 0.5)]:
        ...
        for filt_name, filt_obj in [
            ("hard_cutoff", HardCutoffFilter()),
            ("powerlaw", _powerlaw_filter_for_substrate(sub, d0=float(D_rgg))),
        ]:
            configs.append(RunConfig(
                substrate=sub, filter_obj=filt_obj,
