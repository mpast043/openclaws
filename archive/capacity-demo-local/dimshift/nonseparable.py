...
def run_nonseparable_sweep(
    ...
    if verbose:
        print(f"  Rewired {graph['n_rewired']}/{graph['n_local_edges']} edges")
    ...
    for i, C_geo in enumerate(C_geo_values):
        ...
        print(
            f"    [start lnP] C_geo={C_geo:.4f} nnz={nnz} "
            f"density={density:.3e} avg_deg={avg_degree:.2f}",
            flush=True,
        )
        t_heat = time.time()
        ln_P = log_return_probability_sparse(
            L, sigma_values,
            n_probes=n_probes, m_lanczos=m_lanczos, seed=seed,
        )
        heat_s = time.time() - t_heat
        ln_P_matrix[i] = ln_P
        ...
        print(
            f"  C_geo={C_geo:.4f}  ds_plateau={median:.3f}  "
            f"iqr={iqr:.3f}  min_w={min_active_w_arr[i]:.3f}  "
            f"method={method}  [{i+1}/{n_C}]\n"
            f"    nnz={nnz}  density={density:.3e}  avg_deg={avg_degree:.2f}  "
            f"timing: build={build_s:.3f}s lnP={heat_s:.3f}s ds={ds_s:.3f}s plateau={plateau_s:.3f}s step={step_s:.3f}s",
            flush=True,
        )
    ...
    result = NonseparableResult(
        D=D, N=N, rewire_rate=rewire_rate, seed=seed,
...
def evaluate_criteria(
    ...
    diffs = np.diff(ds_p)
    drops = -diffs[diffs < 0]
    max_drop = float(np.max(drops)) if len(drops) > 0 else 0.0
    criteria["A"] = {
        "name": "Monotonicity",
...
def write_artifacts(
    ...
    with open(p, "w", newline="") as f:
        writer = csv.writer(f)
        header = ["C_geo", "ds_plateau", "plateau_iqr", "min_active_w"]
