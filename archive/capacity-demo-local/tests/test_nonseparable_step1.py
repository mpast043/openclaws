...
class TestAcceptanceCriteria:
    """Acceptance criteria evaluation on a small test case."""

    def test_acceptance_criteria(self):
        result = run_nonseparable_sweep(
            D=TEST_D, N=TEST_N, rewire_rate=TEST_R, seed=TEST_SEED,
            C_geo_steps=30, n_sigma=150,
        )
        criteria = evaluate_criteria(result, mono_tol=0.25)

        # Criterion A with relaxed tolerance for N=16 coarse test
        assert criteria["A"]["pass"], (
            f"Criterion A failed: max_drop={criteria['A']['max_drop']}, "
            f"tol={criteria['A']['tolerance']}"
        )

        # B and C may be marginal at N=16, just warn
        if not criteria["B"].get("pass"):
            import warnings
            warnings.warn(f"Criterion B marginal at N={TEST_N}: {criteria['B']}")

        if not criteria["C"].get("pass"):
            import warnings
            warnings.warn(f"Criterion C marginal at N={TEST_N}: {criteria['C']}")
